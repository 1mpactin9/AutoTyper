// ==UserScript==
// @name         TypingClub AutoTyper
// @namespace    https://typingclub.com/
// @version      1.0.3
// @description  Automates TypingClub lessons. Auto & AFK modes with smart scraping, progress tracking, and persistent skip logs.
// @author       https://github.com/1mpactin9/AutoTyper
// @match        *://*.typingclub.com/*
// @match        *://*.edclub.com/*
// @grant        none
// @run-at       document-idle
// ==/UserScript==

(function () {
  "use strict";

  const EXECUTABLE_SELECTORS = new Set([
    "cmn-code",
    "cmn-definition",
    "cmn-drum",
    "cmn-guide2",
    "cmn-practice",
    "cmn-tip",
    "cmn-travel1",
    "qwerty-caps",
    "qwerty-symbols3",
    "qwerty-z",
    "qwerty-numbers",
  ]);

  const state = {
    running: false,
    queue: [],
    currentLessonIdx: 0,
    processedCount: 0,
    abortController: null,
    scriptNavigating: false,
    liveConfig: null,
    mode: "auto",
  };

  function sleep(ms) {
    return new Promise((resolve) => setTimeout(resolve, ms));
  }

  function getProgramId() {
    const m = location.href.match(/program-(\d+)/);
    return m ? `program-${m[1]}` : "program-unknown";
  }

  function logSkippedLesson(num) {
    const pid = getProgramId();
    const key = `at_skipped_${pid}`;
    try {
      const arr = JSON.parse(localStorage.getItem(key) || "[]");
      if (!arr.includes(num)) arr.push(num);
      localStorage.setItem(key, JSON.stringify(arr.sort((a, b) => a - b)));
    } catch (_) {}
    console.log(
      `[AutoTyper] [AFK] [${pid}] Skipped inexecutable lesson #${num} — ` +
        `all skipped: ${localStorage.getItem(key) || "[]"}`,
    );
  }

  function detectPage() {
    if (document.querySelector(".lparena")) return "MENU";
    if (document.querySelector(".LPVIDEO")) return "VIDEO";
    if (document.getElementById("instruction")) return "UNEXECUTABLE_1";
    if (document.querySelector(".TPGAME")) return "GAME";
    if (
      document.querySelector(".TP_APP1.TPCMN") ||
      document.body.classList.contains("theme-codder")
    ) {
      return document.querySelector(".classic-typing-container")
        ? "EXECUTABLE"
        : "UNEXECUTABLE_2";
    }
    if (document.querySelector(".stars-box")) return "RESULTS";
    return "UNKNOWN";
  }

  function isMenuPage() {
    return detectPage() === "MENU";
  }
  function isResultsPage() {
    return detectPage() === "RESULTS" || !!document.querySelector(".stars-box");
  }

  function getLessonExecutability(boxEl) {
    const icon = boxEl.querySelector(".lesson_icon");
    if (!icon) return false;
    const classes = Array.from(icon.classList);
    const eClass = classes.find((c) => c.startsWith("e-"));
    if (!eClass) return false;
    const base = eClass.split("-")[1];
    const specific = classes.find(
      (c) => c.startsWith(`${base}-`) && c !== eClass,
    );
    if (!specific) return false;
    const fullKey = `${base}-${specific.replace(`${base}-`, "")}`;
    for (const exec of EXECUTABLE_SELECTORS) {
      if (
        fullKey === exec ||
        specific.includes(exec.split("-").slice(1).join("-"))
      )
        return true;
    }
    return false;
  }

  function parseBoxCompletion(box) {
    const hasSilver = !!box.querySelector(".platinum-star");
    const starsEl = box.querySelector(".stars");
    const isCompletedCheck = !!box.querySelector(".completion-check");
    const hasProgress = box.parentElement?.classList.contains("has_progress");

    if (hasSilver) return { starRating: 6, completed: true };
    if (starsEl) {
      const match = starsEl.className.match(/stars-(\d)/);
      if (match) {
        const val = parseInt(match[1]);
        if (val > 0) return { starRating: val, completed: true };
        if (isCompletedCheck || hasProgress)
          return { starRating: 0, completed: true };
      }
    }
    if (isCompletedCheck || hasProgress)
      return { starRating: 0, completed: true };
    return { starRating: null, completed: false };
  }

  function scrapeAutoQueue(min, max) {
    const boxes = document.querySelectorAll(".box");
    const queue = [];
    Array.from(boxes).forEach((box, index) => {
      const numEl = box.querySelector(".lsn_num");
      const num = numEl ? parseInt(numEl.textContent.trim()) : index + 1;
      if (isNaN(num) || num < min || num > max) return;
      const { completed } = parseBoxCompletion(box);
      if (!completed && getLessonExecutability(box)) queue.push(num);
    });
    return queue.sort((a, b) => a - b);
  }

  function scrapeAFKQueue(min, max) {
    const boxes = document.querySelectorAll(".box");
    const queue = [];
    Array.from(boxes).forEach((box, index) => {
      const numEl = box.querySelector(".lsn_num");
      const num = numEl ? parseInt(numEl.textContent.trim()) : index + 1;
      if (isNaN(num) || num < min || num > max) return;
      const { completed } = parseBoxCompletion(box);
      if (!completed) queue.push(num);
    });
    return queue.sort((a, b) => a - b);
  }

  function goToMenu() {
    if (isMenuPage()) return true;
    state.scriptNavigating = true;
    const menuBtn = document.querySelector(".menu-btn");
    if (menuBtn) {
      menuBtn.click();
      setTimeout(() => {
        state.scriptNavigating = false;
      }, 500);
      return true;
    }
    const match = location.href.match(
      /^(https?:\/\/[^/]+\/sportal\/program-\d+)/,
    );
    if (match) {
      location.href = match[1] + ".game";
      return true;
    }
    state.scriptNavigating = false;
    return false;
  }

  function clickLesson(num) {
    const arena = document.querySelector(".lparena");
    if (!arena) return false;
    const target = Array.from(arena.querySelectorAll(".lsn_num")).find(
      (el) => el.innerText.trim() === String(num),
    );
    if (!target) return false;
    const box = target.closest(".box-container") || target.closest(".box");
    if (!box) return false;
    box.click();
    return true;
  }

  const ALPHABET = "abcdefghijklmnopqrstuvwxyz";

  function getRandomWrongChar(correctChar) {
    let c = ALPHABET[Math.floor(Math.random() * ALPHABET.length)];
    return c === correctChar ? (c === "a" ? "b" : "a") : c;
  }

  function getChars() {
    return Array.from(
      document.querySelectorAll(".token span.token_unit"),
      (el) => {
        if (
          el.firstChild?.classList?.contains("_enter") ||
          el.classList.contains("_enter")
        )
          return "\n";
        const char = el.textContent[0];
        return char === "\u00A0" ? " " : char;
      },
    );
  }

  function typeChar(inputField, char) {
    inputField.focus();
    const isEnter = char === "\n";
    const isSpace = char === " ";
    const isBackspace = char === "Backspace";
    const eventKey = isEnter ? "Enter" : isSpace ? " " : char;
    const eventCode = isEnter
      ? "Enter"
      : isSpace
        ? "Space"
        : isBackspace
          ? "Backspace"
          : `Key${char.toUpperCase()}`;
    const eventKeyCode = isEnter
      ? 13
      : isSpace
        ? 32
        : isBackspace
          ? 8
          : char.charCodeAt(0);
    const opts = {
      key: eventKey,
      code: eventCode,
      keyCode: eventKeyCode,
      which: eventKeyCode,
      bubbles: true,
      cancelable: true,
    };

    inputField.dispatchEvent(new KeyboardEvent("keydown", opts));
    if (!isEnter) {
      const nativeSetter = Object.getOwnPropertyDescriptor(
        window.HTMLInputElement.prototype,
        "value",
      )?.set;
      const newValue = isBackspace
        ? inputField.value.slice(0, -1)
        : inputField.value + char;
      if (nativeSetter) {
        nativeSetter.call(inputField, newValue);
      } else {
        inputField.value = newValue;
      }
      inputField.dispatchEvent(
        new InputEvent("input", {
          data: isBackspace ? null : char,
          inputType: isBackspace ? "deleteContentBackward" : "insertText",
          bubbles: true,
        }),
      );
    }
    inputField.dispatchEvent(new KeyboardEvent("keyup", opts));
  }

  async function runTypingSession(cfg, abortSignal) {
    const inputField =
      document.querySelector('input[aria-hidden="true"]') ||
      document.activeElement;
    if (!inputField) return false;

    const characters = getChars();
    if (characters.length === 0) return false;

    const N = characters.length;
    const liveCfg = getConfig();
    const fakeValid = Math.floor((liveCfg.fakeAccuracy / 100) * N);
    const realValid = Math.min(
      Math.floor((liveCfg.realAccuracy / 100) * N),
      fakeValid,
    );

    const actions = [
      ...Array(realValid).fill("A"),
      ...Array(N - fakeValid).fill("B"),
      ...Array(fakeValid - realValid).fill("C"),
    ];
    for (let i = actions.length - 1; i > 0; i--) {
      const j = Math.floor(Math.random() * (i + 1));
      [actions[i], actions[j]] = [actions[j], actions[i]];
    }

    const sequence = [];
    let totalWeight = 0;
    for (let i = 0; i < N; i++) {
      const char = characters[i];
      if (actions[i] === "A") {
        sequence.push({ char, w: 1 });
        totalWeight += 1;
      } else if (actions[i] === "B") {
        sequence.push({ char: getRandomWrongChar(char), w: 1 });
        totalWeight += 1;
      } else {
        sequence.push({ char: getRandomWrongChar(char), w: 0.8 });
        sequence.push({ char: "Backspace", w: 0.4 });
        sequence.push({ char, w: 1.5 });
        totalWeight += 2.7;
      }
    }

    const getTargetDuration = () => (realValid * 12000) / getConfig().wpm;
    let expectedTime = Date.now();

    for (let i = 0; i < sequence.length; i++) {
      if (abortSignal?.aborted) return false;
      if (i === 0) {
        expectedTime = Date.now();
      } else {
        const delay =
          (sequence[i].w / totalWeight) *
          getTargetDuration() *
          (1 + (Math.random() * 0.4 - 0.2));
        expectedTime += delay;
        const wait = expectedTime - Date.now();
        if (wait > 0) await sleep(wait);
      }
      typeChar(inputField, sequence[i].char);
    }
    return true;
  }

  function waitForPage(pageType, timeoutMs = 20000) {
    return new Promise((resolve, reject) => {
      if (detectPage() === pageType) return resolve();
      const obs = new MutationObserver(() => {
        if (detectPage() === pageType) {
          obs.disconnect();
          resolve();
        }
      });
      obs.observe(document.body, {
        childList: true,
        subtree: true,
        attributes: true,
      });
      setTimeout(() => {
        obs.disconnect();
        reject(new Error(`Timeout: ${pageType}`));
      }, timeoutMs);
    });
  }

  function waitForResults(timeoutMs = 60000) {
    return new Promise((resolve, reject) => {
      if (isResultsPage()) return resolve();
      const obs = new MutationObserver(() => {
        if (isResultsPage()) {
          obs.disconnect();
          resolve();
        }
      });
      obs.observe(document.body, { childList: true, subtree: true });
      setTimeout(() => {
        obs.disconnect();
        reject(new Error("Timeout: results"));
      }, timeoutMs);
    });
  }

  function readConfigFromUI() {
    return {
      wpm: parseInt(document.getElementById("at-wpm")?.value) || 80,
      realAccuracy:
        parseFloat(document.getElementById("at-real-acc")?.value) || 100,
      fakeAccuracy:
        parseFloat(document.getElementById("at-fake-acc")?.value) || 98,
    };
  }
  function getConfig() {
    return state.liveConfig || readConfigFromUI();
  }

  function updateProgress() {
    const total = state.queue.length;
    const done = state.processedCount;
    const pct = total > 0 ? (done / total) * 100 : 0;

    const fill = document.getElementById("at-progress-fill");
    const label = document.getElementById("at-progress-label");
    const pctEl = document.getElementById("at-progress-pct");
    if (fill) fill.style.width = pct.toFixed(1) + "%";
    if (label)
      label.textContent = total > 0 ? `${done} / ${total} lessons` : "—";
    if (pctEl) pctEl.textContent = total > 0 ? `${Math.round(pct)}%` : "0%";
  }

  function setStatusBadge(text) {
    const badge = document.getElementById("at-status-badge");
    if (!badge) return;
    badge.textContent = text;
    badge.classList.toggle("visible", text !== "IDLE");
  }

  function updateLessonBadge(idx) {
    setStatusBadge(idx === null ? "IDLE" : `#${state.queue[idx]}`);
  }
  function updateStartStopBtn(running) {
    const btn = document.getElementById("at-btn-start");
    if (!btn) return;
    btn.innerHTML = running ? "■ &nbsp;Stop" : "▶ &nbsp;Start";
    btn.classList.toggle("running", running);
  }
  function stopAutomation() {
    state.running = false;
    state.abortController?.abort();
    updateStartStopBtn(false);
    setStatusBadge("IDLE");
  }

  async function startAutomation() {
    state.liveConfig = readConfigFromUI();
    state.running = true;
    state.processedCount = 0;
    updateStartStopBtn(true);
    setStatusBadge("NAVIGATING…");

    if (!isMenuPage()) {
      goToMenu();
      try {
        await waitForPage("MENU", 15000);
      } catch {
        console.log("[AutoTyper] Could not reach menu — aborting.");
        stopAutomation();
        return;
      }
      await sleep(1000);
    }

    setStatusBadge("SCRAPING…");
    const queue =
      state.mode === "afk" ? scrapeAFKQueue(1, 9999) : scrapeAutoQueue(1, 9999);

    if (queue.length === 0) {
      console.log("[AutoTyper] Queue is empty — nothing to do.");
      stopAutomation();
      return;
    }

    state.queue = queue;
    state.currentLessonIdx = 0;
    updateProgress();
    console.log(
      `[AutoTyper] ${state.mode.toUpperCase()} mode — ${queue.length} lesson(s) queued: [${queue.join(", ")}]`,
    );

    await sleep(400);

    runAutomation().catch(() => stopAutomation());
  }

  async function runAutomation() {
    state.abortController = new AbortController();
    const sig = state.abortController.signal;

    for (let i = state.currentLessonIdx; i < state.queue.length; i++) {
      if (sig.aborted || !state.running) break;
      state.currentLessonIdx = i;

      const lessonNum = state.queue[i];
      updateLessonBadge(i);

      let pageType;
      if (detectPage() === "EXECUTABLE") {
        pageType = "EXECUTABLE";
      } else {
        if (!isMenuPage()) {
          goToMenu();
          try {
            await waitForPage("MENU", 15000);
          } catch {
            break;
          }
          await sleep(800);
        }

        if (!clickLesson(lessonNum)) {
          state.processedCount++;
          updateProgress();
          continue;
        }

        await sleep(3000);
        pageType = detectPage();
      }

      if (pageType !== "EXECUTABLE") {
        if (state.mode === "afk") {
          logSkippedLesson(lessonNum);
        }
        state.processedCount++;
        updateProgress();
        if (!isMenuPage()) {
          goToMenu();
          try {
            await waitForPage("MENU", 15000);
          } catch {
            break;
          }
          await sleep(800);
        }
        continue;
      }

      const success = await runTypingSession(getConfig(), sig);
      if (!success) {
        if (!state.running) break;
        continue;
      }

      try {
        await waitForResults(90000);
      } catch {
        state.processedCount++;
        updateProgress();
        continue;
      }

      await sleep(1500);
      state.processedCount++;
      updateProgress();
      const nextLesson = state.queue[i + 1];
      if (nextLesson && nextLesson === lessonNum + 1) {
        await sleep(600);
        document.dispatchEvent(
          new KeyboardEvent("keydown", {
            key: "Enter",
            code: "Enter",
            keyCode: 13,
            bubbles: true,
          }),
        );
        document.dispatchEvent(
          new KeyboardEvent("keyup", {
            key: "Enter",
            code: "Enter",
            keyCode: 13,
            bubbles: true,
          }),
        );

        const transitioned = await new Promise((resolve) => {
          const start = Date.now();
          const check = setInterval(() => {
            const pg = detectPage();
            if (
              [
                "EXECUTABLE",
                "MENU",
                "VIDEO",
                "GAME",
                "UNEXECUTABLE_1",
                "UNEXECUTABLE_2",
              ].includes(pg)
            ) {
              clearInterval(check);
              resolve(pg);
            }
            if (Date.now() - start > 8000) {
              clearInterval(check);
              resolve(null);
            }
          }, 200);
        });

        if (!transitioned || transitioned === "MENU") {
          if (!transitioned) {
            goToMenu();
            try {
              await waitForPage("MENU", 15000);
            } catch {
              break;
            }
          }
          await sleep(800);
        }
      } else {
        await sleep(500);
        goToMenu();
        try {
          await waitForPage("MENU", 15000);
        } catch {
          break;
        }
        await sleep(800);
      }
    }

    stopAutomation();
  }

  function setupInterferenceDetection() {
    document.addEventListener(
      "click",
      (e) => {
        if (!state.running || state.scriptNavigating) return;
        if (e.target.closest(".menu-btn")) stopAutomation();
      },
      true,
    );

    document.addEventListener(
      "keydown",
      (e) => {
        if (!state.running || !e.isTrusted) return;
        const typingInput = document.querySelector('input[aria-hidden="true"]');
        if (typingInput && e.target === typingInput) stopAutomation();
      },
      true,
    );
  }

  function buildUI() {
    document.getElementById("at-panel")?.remove();

    const panel = document.createElement("div");
    panel.id = "at-panel";
    panel.innerHTML = `
    <style>
      @import url('https://fonts.googleapis.com/css2?family=Inter:wght@400;500;600&family=JetBrains+Mono:wght@400;600&display=swap');

      /* ── PANEL ── */
      #at-panel {
        position: fixed; z-index: 2147483647;
        bottom: 28px; right: 28px;
        width: 282px; min-width: 220px; max-width: 520px;
        font-family: 'Inter', system-ui, sans-serif; font-size: 12px;
        background: rgba(10,10,14,0.97);
        border: 1px solid rgba(255,255,255,0.08); border-radius: 14px;
        box-shadow: 0 24px 60px rgba(0,0,0,0.8), 0 0 0 1px rgba(255,255,255,0.03) inset;
        color: #b0b0c0; overflow: visible;
        backdrop-filter: blur(12px); -webkit-backdrop-filter: blur(12px);
        transition: opacity 0.2s, transform 0.2s;
        user-select: none;
      }
      #at-panel * { box-sizing: border-box; }

      /* snap edge indicators */
      #at-panel.snap-left   { border-left:   2px solid rgba(124,111,255,0.35); border-radius: 0 14px 14px 0; }
      #at-panel.snap-right  { border-right:  2px solid rgba(124,111,255,0.35); border-radius: 14px 0 0 14px; }
      #at-panel.snap-top    { border-top:    2px solid rgba(124,111,255,0.35); border-radius: 0 0 14px 14px; }
      #at-panel.snap-bottom { border-bottom: 2px solid rgba(124,111,255,0.35); border-radius: 14px 14px 0 0; }

      /* ── TITLEBAR ── */
      #at-titlebar {
        display: flex; align-items: center; gap: 8px;
        padding: 11px 13px; cursor: grab;
        background: rgba(255,255,255,0.025);
        border-bottom: 1px solid rgba(255,255,255,0.06);
        border-radius: 14px 14px 0 0;
      }
      #at-titlebar:active { cursor: grabbing; }
      .at-dot { width: 8px; height: 8px; border-radius: 50%; flex-shrink: 0; }
      .at-dot-red   { background: #ff5f57; }
      .at-dot-amber { background: #ffbd2e; }
      .at-dot-green { background: #28c840; }
      #at-title-text {
        font-family: 'JetBrains Mono', monospace; font-size: 10px; font-weight: 600;
        letter-spacing: 1.5px; color: rgba(255,255,255,0.5); text-transform: uppercase; flex: 1;
      }
      #at-status-badge {
        font-family: 'JetBrains Mono', monospace; font-size: 9px; font-weight: 600;
        padding: 2px 6px; border-radius: 4px;
        background: rgba(124,111,255,0.15); color: #7c6fff;
        letter-spacing: 0.5px; display: none;
      }
      #at-status-badge.visible { display: inline-block; }
      #at-collapse-btn {
        background: none; border: none; cursor: pointer;
        color: rgba(255,255,255,0.25); font-size: 14px;
        padding: 0; line-height: 1; transition: color 0.15s; flex-shrink: 0;
      }
      #at-collapse-btn:hover { color: rgba(255,255,255,0.6); }

      /* ── BODY ── */
      #at-body {
        padding: 13px 13px 18px;
        display: flex; flex-direction: column; gap: 10px;
        overflow: hidden; border-radius: 0 0 14px 14px;
      }
      #at-panel.at-collapsed #at-body { display: none; }

      /* ── MODE TOGGLE ── */
      #at-mode-toggle {
        display: flex; border-radius: 8px; overflow: hidden;
        border: 1px solid rgba(255,255,255,0.08);
        background: rgba(255,255,255,0.03);
      }
      .at-mode-btn {
        flex: 1; padding: 7px 0; background: none; border: none;
        cursor: pointer; font-family: 'Inter', sans-serif;
        font-size: 11px; font-weight: 600; letter-spacing: 0.2px;
        color: rgba(255,255,255,0.22); transition: all 0.15s;
      }
      .at-mode-btn:first-child { border-right: 1px solid rgba(255,255,255,0.06); }
      .at-mode-btn.auto-active { background: rgba(124,111,255,0.15); color: #9b8fff; }
      .at-mode-btn.afk-active  { background: rgba(255,184,0,0.12);   color: #ffb800; }

      /* ── START BTN ── */
      #at-btn-start {
        width: 100%; padding: 9px; border: none; border-radius: 8px; cursor: pointer;
        font-family: 'Inter', sans-serif; font-size: 12px; font-weight: 600; letter-spacing: 0.3px;
        background: linear-gradient(135deg, #7c6fff 0%, #5548d9 100%);
        color: #fff; box-shadow: 0 2px 16px rgba(124,111,255,0.35);
        transition: all 0.18s;
      }
      #at-btn-start:hover { box-shadow: 0 4px 24px rgba(124,111,255,0.55); transform: translateY(-1px); }
      #at-btn-start:active { transform: translateY(0); }
      #at-btn-start.running {
        background: linear-gradient(135deg, #ff4f6f 0%, #b0263e 100%);
        box-shadow: 0 2px 16px rgba(255,79,111,0.35);
      }
      #at-btn-start.running:hover { box-shadow: 0 4px 24px rgba(255,79,111,0.55); }

      /* ── DIVIDER ── */
      .at-sep { height: 1px; background: rgba(255,255,255,0.055); margin: 1px 0; }

      /* ── LABELS ── */
      .at-section-label {
        font-size: 9px; font-weight: 600; letter-spacing: 1.2px;
        text-transform: uppercase; color: rgba(255,255,255,0.2); margin-bottom: -4px;
      }

      /* ── SLIDERS ── */
      .at-slider-group { display: flex; flex-direction: column; gap: 7px; }
      .at-slider-row   { display: flex; flex-direction: column; gap: 4px; }
      .at-slider-header { display: flex; justify-content: space-between; align-items: baseline; }
      .at-slider-label { font-size: 10px; font-weight: 500; color: rgba(255,255,255,0.4); }
      .at-slider-value {
        font-family: 'JetBrains Mono', monospace; font-size: 11px; font-weight: 600;
        color: rgba(255,255,255,0.75); min-width: 42px; text-align: right;
      }
      .at-slider {
        -webkit-appearance: none; appearance: none;
        width: 100%; height: 3px; border-radius: 2px; outline: none; cursor: pointer;
        background: rgba(255,255,255,0.1); transition: background 0.2s;
      }
      .at-slider::-webkit-slider-thumb {
        -webkit-appearance: none; appearance: none;
        width: 13px; height: 13px; border-radius: 50%;
        background: #7c6fff; box-shadow: 0 0 0 2px rgba(124,111,255,0.3);
        cursor: grab; transition: transform 0.15s, box-shadow 0.15s;
      }
      .at-slider::-webkit-slider-thumb:active { cursor: grabbing; transform: scale(1.2); }
      .at-slider::-moz-range-thumb {
        width: 13px; height: 13px; border-radius: 50%; border: none;
        background: #7c6fff; cursor: grab;
      }
      .at-slider.acc-slider::-webkit-slider-thumb { background: #06d6a0; box-shadow: 0 0 0 2px rgba(6,214,160,0.3); }
      .at-slider.acc-slider::-moz-range-thumb { background: #06d6a0; }

      /* ── PROGRESS ── */
      .at-progress-track {
        background: rgba(255,255,255,0.05); border-radius: 6px;
        overflow: hidden; height: 5px;
      }
      #at-progress-fill {
        height: 100%; width: 0%;
        background: linear-gradient(90deg, #5548d9, #7c6fff, #a89fff);
        border-radius: 6px;
        transition: width 0.5s cubic-bezier(0.22, 1, 0.36, 1);
      }
      .at-progress-meta {
        display: flex; justify-content: space-between; align-items: center;
        margin-top: 4px;
      }
      #at-progress-label {
        font-family: 'JetBrains Mono', monospace; font-size: 9px;
        color: rgba(255,255,255,0.25);
      }
      #at-progress-pct {
        font-family: 'JetBrains Mono', monospace; font-size: 10px;
        font-weight: 600; color: #7c6fff;
      }

      /* ── RESIZE HANDLES ── */
      #at-resize-h {
        position: absolute; bottom: 0; left: 14px; right: 14px;
        height: 8px; cursor: ns-resize;
      }
      #at-resize-h::after {
        content: ''; display: block; width: 28px; height: 2px;
        border-radius: 1px; background: rgba(255,255,255,0.07);
        margin: 3px auto 0;
      }
      #at-resize-w {
        position: absolute; top: 40px; right: -4px; bottom: 8px;
        width: 8px; cursor: ew-resize;
      }
      #at-resize-w::after {
        content: ''; display: block; width: 2px; height: 24px;
        border-radius: 1px; background: rgba(255,255,255,0.07);
        margin: 50% 3px;
      }
      #at-resize-w:hover::after { background: rgba(124,111,255,0.4); }
    </style>

    <!-- TITLEBAR -->
    <div id="at-titlebar">
      <span class="at-dot at-dot-red"   id="at-dot-red"   title="Hide (Alt+T to restore)" style="cursor:pointer"></span>
      <span class="at-dot at-dot-amber" id="at-dot-amber" title="Collapse" style="cursor:pointer"></span>
      <span class="at-dot at-dot-green" id="at-dot-green" style="cursor:default;opacity:0.3"></span>
      <span id="at-title-text">AutoTyper</span>
      <span id="at-status-badge">IDLE</span>
      <button id="at-collapse-btn" title="Collapse">▾</button>
    </div>

    <!-- BODY -->
    <div id="at-body">

      <!-- Mode Toggle -->
      <div id="at-mode-toggle">
        <button class="at-mode-btn auto-active" id="at-mode-auto">⚡ Auto Mode</button>
        <button class="at-mode-btn"             id="at-mode-afk">💤 AFK Mode</button>
      </div>

      <!-- Start / Stop -->
      <button id="at-btn-start">▶ &nbsp;Start</button>

      <div class="at-sep"></div>

      <!-- Speed -->
      <div class="at-section-label">Speed</div>
      <div class="at-slider-group">
        <div class="at-slider-row">
          <div class="at-slider-header">
            <span class="at-slider-label">Target WPM</span>
            <span class="at-slider-value" id="at-wpm-val">80 wpm</span>
          </div>
          <input type="range" class="at-slider" id="at-wpm" min="20" max="250" value="80" step="1">
        </div>
      </div>

      <!-- Accuracy -->
      <div class="at-section-label">Accuracy</div>
      <div class="at-slider-group">
        <div class="at-slider-row">
          <div class="at-slider-header">
            <span class="at-slider-label">Fake Accuracy</span>
            <span class="at-slider-value" id="at-fake-acc-val">100.0%</span>
          </div>
          <input type="range" class="at-slider acc-slider" id="at-fake-acc" min="80" max="100" value="100" step="0.5">
        </div>
        <div class="at-slider-row">
          <div class="at-slider-header">
            <span class="at-slider-label">Real Accuracy</span>
            <span class="at-slider-value" id="at-real-acc-val">98.0%</span>
          </div>
          <input type="range" class="at-slider acc-slider" id="at-real-acc" min="80" max="100" value="98" step="0.5">
        </div>
      </div>

      <div class="at-sep"></div>

      <!-- Progress -->
      <div class="at-section-label">Progress</div>
      <div class="at-progress-track">
        <div id="at-progress-fill"></div>
      </div>
      <div class="at-progress-meta">
        <span id="at-progress-label">—</span>
        <span id="at-progress-pct">0%</span>
      </div>

    </div><!-- /#at-body -->

    <!-- Resize handles (outside body so they're always accessible) -->
    <div id="at-resize-h"></div>
    <div id="at-resize-w"></div>
    `;

    document.body.appendChild(panel);

    const titlebar = panel.querySelector("#at-titlebar");
    let dragging = false,
      ox = 0,
      oy = 0;
    const SNAP = 14;

    titlebar.addEventListener("mousedown", (e) => {
      if (e.target.closest("button") || e.target.classList.contains("at-dot"))
        return;
      dragging = true;
      const r = panel.getBoundingClientRect();
      ox = e.clientX - r.left;
      oy = e.clientY - r.top;
      panel.style.transition = "none";
      e.preventDefault();
    });

    document.addEventListener("mousemove", (e) => {
      if (!dragging) return;
      let nx = e.clientX - ox;
      let ny = e.clientY - oy;
      const maxX = window.innerWidth - panel.offsetWidth;
      const maxY = window.innerHeight - panel.offsetHeight;
      nx = Math.max(0, Math.min(maxX, nx));
      ny = Math.max(0, Math.min(maxY, ny));

      panel.classList.remove(
        "snap-left",
        "snap-right",
        "snap-top",
        "snap-bottom",
      );
      if (nx <= SNAP) {
        nx = 0;
        panel.classList.add("snap-left");
      }
      if (nx >= maxX - SNAP) {
        nx = maxX;
        panel.classList.add("snap-right");
      }
      if (ny <= SNAP) {
        ny = 0;
        panel.classList.add("snap-top");
      }
      if (ny >= maxY - SNAP) {
        ny = maxY;
        panel.classList.add("snap-bottom");
      }

      panel.style.left = nx + "px";
      panel.style.top = ny + "px";
      panel.style.right = "auto";
      panel.style.bottom = "auto";
    });

    document.addEventListener("mouseup", () => {
      if (dragging) {
        panel.style.transition = "";
        dragging = false;
      }
    });

    const resizeH = panel.querySelector("#at-resize-h");
    let resizingH = false,
      hStartY = 0,
      hStartH = 0;

    resizeH.addEventListener("mousedown", (e) => {
      resizingH = true;
      hStartY = e.clientY;
      hStartH = panel.offsetHeight;
      panel.style.transition = "none";
      e.preventDefault();
    });
    document.addEventListener("mousemove", (e) => {
      if (!resizingH) return;
      const newH = Math.max(180, hStartH + (e.clientY - hStartY));
      panel.style.height = newH + "px";
      panel.style.maxHeight = newH + "px";
    });
    document.addEventListener("mouseup", () => {
      if (resizingH) {
        panel.style.transition = "";
        resizingH = false;
      }
    });

    const resizeW = panel.querySelector("#at-resize-w");
    let resizingW = false,
      wStartX = 0,
      wStartW = 0;

    resizeW.addEventListener("mousedown", (e) => {
      resizingW = true;
      wStartX = e.clientX;
      wStartW = panel.offsetWidth;
      panel.style.transition = "none";
      e.preventDefault();
    });
    document.addEventListener("mousemove", (e) => {
      if (!resizingW) return;
      const newW = Math.max(
        220,
        Math.min(540, wStartW + (e.clientX - wStartX)),
      );
      panel.style.width = newW + "px";
    });
    document.addEventListener("mouseup", () => {
      if (resizingW) {
        panel.style.transition = "";
        resizingW = false;
      }
    });

    document.getElementById("at-dot-red").addEventListener("click", () => {
      panel.style.opacity = "0";
      panel.style.pointerEvents = "none";
      panel.style.transform = "scale(0.92)";

      const hint = document.createElement("div");
      hint.style.cssText = `
        position:fixed;bottom:28px;right:28px;z-index:2147483646;
        font-family:'JetBrains Mono',monospace;font-size:10px;
        color:rgba(255,255,255,0.4);background:rgba(10,10,14,0.9);
        padding:6px 12px;border-radius:8px;border:1px solid rgba(255,255,255,0.07);
        cursor:pointer;letter-spacing:0.5px;
      `;
      hint.textContent = "Alt+T — restore AutoTyper";
      document.body.appendChild(hint);

      const restore = () => {
        panel.style.opacity = "1";
        panel.style.pointerEvents = "";
        panel.style.transform = "";
        hint.remove();
        document.removeEventListener("keydown", onKey);
      };
      const onKey = (e) => {
        if (e.altKey && e.key === "t") restore();
      };
      hint.addEventListener("click", restore);
      document.addEventListener("keydown", onKey);
    });

    const collapseBtn = panel.querySelector("#at-collapse-btn");
    let collapsed = false;
    const doCollapse = () => {
      collapsed = !collapsed;
      panel.classList.toggle("at-collapsed", collapsed);
      collapseBtn.textContent = collapsed ? "▸" : "▾";
    };
    document
      .getElementById("at-dot-amber")
      .addEventListener("click", doCollapse);
    collapseBtn.addEventListener("click", doCollapse);

    function wireSlider(id, displayId, suffix = "", decimals = 0) {
      const slider = document.getElementById(id);
      const display = document.getElementById(displayId);
      const fmt = (v) =>
        decimals > 0
          ? parseFloat(v).toFixed(decimals) + suffix
          : parseInt(v) + suffix;

      const updateTrack = () => {
        const pct =
          ((slider.value - slider.min) / (slider.max - slider.min)) * 100;
        const color = id.includes("acc") ? "#06d6a0" : "#7c6fff";
        slider.style.background = `linear-gradient(to right, ${color} ${pct}%, rgba(255,255,255,0.1) ${pct}%)`;
        display.textContent = fmt(slider.value);
      };

      let debounce = null;
      slider.addEventListener("input", () => {
        updateTrack();
        clearTimeout(debounce);
        debounce = setTimeout(() => {
          state.liveConfig = readConfigFromUI();
        }, 600);
      });
      updateTrack();
    }
    wireSlider("at-wpm", "at-wpm-val", " wpm");
    wireSlider("at-fake-acc", "at-fake-acc-val", "%", 1);
    wireSlider("at-real-acc", "at-real-acc-val", "%", 1);

    const modeAutoBtn = document.getElementById("at-mode-auto");
    const modeAfkBtn = document.getElementById("at-mode-afk");

    modeAutoBtn.addEventListener("click", () => {
      if (state.running) return;
      state.mode = "auto";
      modeAutoBtn.className = "at-mode-btn auto-active";
      modeAfkBtn.className = "at-mode-btn";
    });
    modeAfkBtn.addEventListener("click", () => {
      if (state.running) return;
      state.mode = "afk";
      modeAfkBtn.className = "at-mode-btn afk-active";
      modeAutoBtn.className = "at-mode-btn";
    });

    document.getElementById("at-btn-start").addEventListener("click", () => {
      if (state.running) {
        stopAutomation();
      } else {
        startAutomation().catch(() => stopAutomation());
      }
    });
  }

  function init() {
    if (
      !location.href.includes("typingclub.com") &&
      !location.href.includes("edclub.com")
    )
      return;
    buildUI();
    setupInterferenceDetection();
  }

  if (document.readyState === "loading") {
    document.addEventListener("DOMContentLoaded", init);
  } else {
    init();
  }
})();
